...
export default function DashboardProducts({
  ...
  // fetch first time or reload
  useEffect(() => {
    fetchData();
  }, [currentPage, keyword]);

    return (
        <div className="gap-6 bg-zinc-50 dark:bg-zinc-900  pb-10">
            <div className="flex flex-col rounded-2xl bg-white dark:bg-zinc-800 px-6 py-9">
                {/* Page Header + CTA Button */}
                <div className="mb-6 flex items-center justify-between">
                    <h1 className="text-2xl font-semibold dark:text-zinc-50">{t("all-products")}</h1>
                    <Button type="button" className="flex items-center gap-2 text-base font-medium" onClick={() => setStep("new_product")}>
                        <Plus className="!h-5 !w-5 shrink-0" size={32} /> {t("add-product")}
                    </Button>
                </div>
                ...
                {!productData && isPending ? (
                    ...
                    !("error" in productData) && (
                        <ProductsTable products={productData.products} setProductId={setProductId} setStep={setStep} onDelete={fetchData} />
                    )
                )}
            </div>
